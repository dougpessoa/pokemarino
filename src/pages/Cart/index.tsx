...
const Cart: React.FC = () => {
  ...
  function handleFinishShop(total: number, cashback: number) {
    ...
    const purchased = localStorage.getItem("@Pokemarino: purchased");
    ...
    localStorage.setItem("@Pokemarino: purchased", JSON.stringify(arrayPurchased));

    localStorage.removeItem('@Pokemarino: products');
        ...
        <List>
          {
            listOfCart.length === 0 
              ? <EmptyCart>
                  ...
                  <p>
                    Só tem água! <br />
                    No momento não há pokémon no seu carrinho.<br />
                    Vamos as compras? 
                  </p>
                </EmptyCart>
